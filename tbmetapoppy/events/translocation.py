...
class BacteriaTranslocationLymphToLung(Translocation):
    ...
    def __init__(self, cell_type):
        Translocation.__init__(self, TBPulmonaryEnvironment.LYMPH_PATCH, cell_type, [TBPulmonaryEnvironment.CASEUM])
    ...
    def _calculate_state_variable_at_patch(self, network, patch_id):
        caseum = network.get_compartment_value(patch_id, TBPulmonaryEnvironment.CASEUM)
        return network.get_compartment_value(patch_id, self._moving_compartment) * \
