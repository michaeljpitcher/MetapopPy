...
class CellInfectionTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.rp = 0.1
        self.event_mac.set_reaction_parameter(self.rp)
        self.event_dc.set_reaction_parameter(self.rp)
    ...
    def test_rate(self):
        self.event_mac.set_parameters(100)
        self.event_dc.set_parameters(88)
        ...
        self.assertEqual(self.event_mac.calculate_rate_at_patch(self.network, 1), self.rp * 9 * (11.0 / (11 + 100)))
        self.assertFalse(self.event_dc.calculate_rate_at_patch(self.network, 1))
        self.network.update_patch(1, compartment_changes={BACTERIUM_EXTRACELLULAR_DORMANT: 6})
        self.assertEqual(self.event_mac.calculate_rate_at_patch(self.network, 1), self.rp * 9 * (17.0 / (17 + 100)))
        self.assertFalse(self.event_dc.calculate_rate_at_patch(self.network, 1))
        self.network.update_patch(1, compartment_changes={DENDRITIC_CELL_IMMATURE: 3})
        self.assertEqual(self.event_mac.calculate_rate_at_patch(self.network, 1), self.rp * 9 * (17.0 / (17 + 100)))
        self.assertEqual(self.event_dc.calculate_rate_at_patch(self.network, 1), self.rp * 3 * (17.0 / (17 + 88)))
