...
class TranslocationLymphToLungTestCase(unittest.TestCase):
    def setUp(self):
        self.event = TranslocationLymphToLungBlood(TBPulmonaryEnvironment.BACTERIUM_EXTRACELLULAR_DORMANT)
        self.params = {'b_ed_translocation_from_lymph_patch_by_blood_rate': 0.1,
                       'b_ed_translocation_from_lymph_patch_by_blood_half_sat': 50}
        self.event.set_parameters(self.params)
    ...
    def test_rate(self):
        ...
        self.assertEqual(self.event.calculate_rate_at_patch(self.network, TBPulmonaryEnvironment.LYMPH_PATCH),
                         self.params['b_ed_translocation_from_lymph_patch_by_blood_rate'] * 13)

    def test_perform(self):
        total_q = sum([n+0.5 for n in range(5)])
        for n in range(5):
            self.network.update_patch(n, attribute_changes={TBPulmonaryEnvironment.PERFUSION: (n + 0.5)/total_q})
        ...
        # TODO - random involved so may fail (probably won't due to large bacteria numbers)
...
#     def setUp(self):
#         self.event = TranslocationLymphToLungCytokine(TBPulmonaryEnvironment.T_CELL_ACTIVATED)
#         self.params = {'t_a_translocation_from_lymph_patch_by_cytokine_rate': 0.1,
#                        't_a_translocation_from_lymph_patch_by_cytokine_sigmoid': 2,
...
#                                self.params['t_a_translocation_from_lymph_patch_by_cytokine_rate'] * 7.0 *
#                                ((13.0)**self.params['t_a_translocation_from_lymph_patch_by_cytokine_sigmoid']) /
#                                ((13.0)**self.params['t_a_translocation_from_lymph_patch_by_cytokine_sigmoid'] +
#                           self.params['t_a_translocation_from_lymph_patch_half_sat']**
#                           self.params['t_a_translocation_from_lymph_patch_by_cytokine_sigmoid']))
#
#         self.network.update_patch(TBPulmonaryEnvironment.LYMPH_PATCH, {TBPulmonaryEnvironment.DENDRITIC_CELL_MATURE: 6})
#         self.assertAlmostEqual(self.event.calculate_rate_at_patch(self.network, TBPulmonaryEnvironment.LYMPH_PATCH),
#                                self.params['t_a_translocation_from_lymph_patch_by_cytokine_rate'] * 7.0 *
#                                ((19.0) ** self.params['t_a_translocation_from_lymph_patch_by_cytokine_sigmoid']) /
#                                ((19.0) ** self.params['t_a_translocation_from_lymph_patch_by_cytokine_sigmoid'] +
#                            self.params['t_a_translocation_from_lymph_patch_half_sat']**
#                            self.params['t_a_translocation_from_lymph_patch_by_cytokine_sigmoid']))
#
