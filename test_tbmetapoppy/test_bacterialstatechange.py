import unittest
from tbmetapoppy.events.bacterial_state_change import *
...
class BacteriumBecomesDormantTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.network = PulmonaryNetwork({PulmonaryNetwork.TOPOLOGY: None}, TBDynamics.TB_COMPARTMENTS)
    ...
    def test_rate(self):
        ...
        self.network.update_patch(1, {TBDynamics.BACTERIUM_EXTRACELLULAR_REPLICATING: 2})
    ...
    def test_perform(self):
        ...
        self.assertEqual(self.network.get_compartment_value(1, TBDynamics.BACTERIUM_EXTRACELLULAR_REPLICATING), 0)
        self.assertEqual(self.network.get_compartment_value(1, TBDynamics.BACTERIUM_EXTRACELLULAR_DORMANT), 1)
...
class BacteriumBecomesReplicatingTestCase(unittest.TestCase):
    ...
    def test_rate(self):
        ...
        self.network.update_patch(1,attribute_changes={PulmonaryNetwork.OXYGEN_TENSION: 0.4})
        self.assertFalse(self.event.calculate_rate_at_patch(self.network, 1))
        self.network.update_patch(1, {TBDynamics.BACTERIUM_EXTRACELLULAR_DORMANT: 2})
    ...
    def test_perform(self):
        self.network.update_patch(1, {TBDynamics.BACTERIUM_EXTRACELLULAR_DORMANT: 1})
        self.event.perform(self.network, 1)
        self.assertEqual(self.network.get_compartment_value(1, TBDynamics.BACTERIUM_EXTRACELLULAR_REPLICATING), 1)
        self.assertEqual(self.network.get_compartment_value(1, TBDynamics.BACTERIUM_EXTRACELLULAR_DORMANT), 0)
