...
class CellDeathTestCase(unittest.TestCase):
    ...
    def setUp(self):
        self.event = CellDeath(TEST_RATE_DEATH_MR, TBPulmonaryNetwork.MACROPHAGE_RESTING)
        self.event_mi = CellDeath(TEST_RATE_DEATH_MI, TBPulmonaryNetwork.MACROPHAGE_INFECTED)
        self.event_dcm = CellDeath(TEST_RATE_DEATH_DCM, TBPulmonaryNetwork.DENDRITIC_CELL_MATURE)
    ...
    def test_rate(self):
        ...
        self.network.update_patch(1, {TBPulmonaryNetwork.MACROPHAGE_RESTING: 1, TBPulmonaryNetwork.MACROPHAGE_INFECTED: 2})
    ...
    def test_perform(self):
        ...
        self.assertEqual(self.network.get_compartment_value(1, TBPulmonaryNetwork.BACTERIUM_INTRACELLULAR_MACROPHAGE), 0)
        self.assertEqual(self.network.get_compartment_value(1, TBPulmonaryNetwork.BACTERIUM_EXTRACELLULAR_DORMANT), 10)
        self.assertEqual(self.network.get_compartment_value(1, TBPulmonaryNetwork.CASEUM), 1)

        # Round down - 10/3
        self.network.update_patch(1, {TBPulmonaryNetwork.BACTERIUM_EXTRACELLULAR_DORMANT: -10,
        ...
        self.assertEqual(self.network.get_compartment_value(1, TBPulmonaryNetwork.BACTERIUM_EXTRACELLULAR_DORMANT), 3)
        self.assertEqual(self.network.get_compartment_value(1, TBPulmonaryNetwork.CASEUM), 2)

        # Round up - 10/4
        self.network.update_patch(1, {TBPulmonaryNetwork.BACTERIUM_EXTRACELLULAR_DORMANT: -3,
        ...
        self.assertEqual(self.network.get_compartment_value(1, TBPulmonaryNetwork.BACTERIUM_INTRACELLULAR_MACROPHAGE), 7)
        self.assertEqual(self.network.get_compartment_value(1, TBPulmonaryNetwork.BACTERIUM_EXTRACELLULAR_DORMANT), 3)
        self.assertEqual(self.network.get_compartment_value(1, TBPulmonaryNetwork.CASEUM), 3)

        self.network.update_patch(1, {TBPulmonaryNetwork.BACTERIUM_EXTRACELLULAR_DORMANT: -3,
                                      TBPulmonaryNetwork.MACROPHAGE_INFECTED: -3,
                                      TBPulmonaryNetwork.BACTERIUM_INTRACELLULAR_MACROPHAGE: -7,
                                      TBPulmonaryNetwork.DENDRITIC_CELL_MATURE: 3,
...
class MacrophageDestroysBacteriumTestCase(unittest.TestCase):
    ...
    def setUp(self):
        self.event = MacrophageDestroysBacterium(TEST_RATE_MACBAC_KILL, TBPulmonaryNetwork.MACROPHAGE_RESTING,
