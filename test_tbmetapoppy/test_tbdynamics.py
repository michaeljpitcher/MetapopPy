...
class TBDynamicsTestCase(unittest.TestCase):
    ...
    def test_initialise(self):
        ...
        self.assertEqual(self.dynamics._rate_table.shape, (31, 17))

        self.assertEqual(len(self.dynamics._events), 31)
        ...
        bed_to_ber = [e for e in self.dynamics._events if isinstance(e, BacteriumBecomesReplicating)]
        self.assertEqual(len(bed_to_ber), 1)
        ber_to_bed = [e for e in self.dynamics._events if isinstance(e, BacteriumBecomesDormant)]
        self.assertEqual(len(ber_to_bed), 1)

        # Bacterial destruction
        mr_kills_ber = [e for e in self.dynamics._events if isinstance(e, MacrophageDestroysBacterium) and
                        e._macrophage_type == MACROPHAGE_RESTING and
                        e._bacterium_type == BACTERIUM_EXTRACELLULAR_REPLICATING]
        mr_kills_bed = [e for e in self.dynamics._events if isinstance(e, MacrophageDestroysBacterium) and
                        e._macrophage_type == MACROPHAGE_RESTING and
                        e._bacterium_type == BACTERIUM_EXTRACELLULAR_DORMANT]
        ma_kills_ber = [e for e in self.dynamics._events if isinstance(e, MacrophageDestroysBacterium) and
                        e._macrophage_type == MACROPHAGE_ACTIVATED and
                        e._bacterium_type == BACTERIUM_EXTRACELLULAR_REPLICATING]
        ma_kills_bed = [e for e in self.dynamics._events if isinstance(e, MacrophageDestroysBacterium) and
                        e._macrophage_type == MACROPHAGE_ACTIVATED and
                        e._bacterium_type == BACTERIUM_EXTRACELLULAR_DORMANT]
        for e in [mr_kills_bed, mr_kills_ber, ma_kills_bed, ma_kills_ber]:
    ...
    def test_seed(self):
        ...
        self.assertEqual(bim_rep._sigmoid, params[TBDynamics.SIGMOID_BIM_REPLICATION])
        self.assertEqual(bim_rep._carrying_capacity, params[TBDynamics.MACROPHAGE_CAPACITY])

        # Bacterial state change
        bed_to_ber = next(e for e in self.dynamics._events if isinstance(e, BacteriumBecomesReplicating))
        self.assertEqual(bed_to_ber._reaction_parameter, params[TBDynamics.RP_CHANGE_TO_REPLICATING])
        self.assertEqual(bed_to_ber._sigmoid, params[TBDynamics.SIGMOID_CHANGE_TO_REPLICATING])
        self.assertEqual(bed_to_ber._half_sat, params[TBDynamics.HALF_SAT_CHANGE_TO_REPLICATING])
        ber_to_bed = next(e for e in self.dynamics._events if isinstance(e, BacteriumBecomesDormant))
        self.assertEqual(ber_to_bed._reaction_parameter, params[TBDynamics.RP_CHANGE_TO_DORMANT])
        self.assertEqual(ber_to_bed._sigmoid, params[TBDynamics.SIGMOID_CHANGE_TO_DORMANT])
        self.assertEqual(ber_to_bed._half_sat, params[TBDynamics.HALF_SAT_CHANGE_TO_DORMANT])

        # Bacterial destruction
        mr_kills_ber = next(e for e in self.dynamics._events if isinstance(e, MacrophageDestroysBacterium) and
                        e._macrophage_type == MACROPHAGE_RESTING and
                        e._bacterium_type == BACTERIUM_EXTRACELLULAR_REPLICATING)
        self.assertEqual(mr_kills_ber._reaction_parameter, params[TBDynamics.RP_MR_DESTROY_BACTERIA])
        self.assertEqual(mr_kills_ber._half_sat, params[TBDynamics.HALF_SAT_MR_DESTROY_BACTERIA])

        mr_kills_bed = next(e for e in self.dynamics._events if isinstance(e, MacrophageDestroysBacterium) and
                        e._macrophage_type == MACROPHAGE_RESTING and
                        e._bacterium_type == BACTERIUM_EXTRACELLULAR_DORMANT)
        self.assertEqual(mr_kills_bed._reaction_parameter, params[TBDynamics.RP_MR_DESTROY_BACTERIA])
        self.assertEqual(mr_kills_bed._half_sat, params[TBDynamics.HALF_SAT_MR_DESTROY_BACTERIA])

        ma_kills_ber = next(e for e in self.dynamics._events if isinstance(e, MacrophageDestroysBacterium) and
                        e._macrophage_type == MACROPHAGE_ACTIVATED and
                        e._bacterium_type == BACTERIUM_EXTRACELLULAR_REPLICATING)
        self.assertEqual(ma_kills_ber._reaction_parameter, params[TBDynamics.RP_MA_DESTROY_BACTERIA])
        self.assertEqual(ma_kills_ber._half_sat, params[TBDynamics.HALF_SAT_MA_DESTROY_BACTERIA])

        ma_kills_bed = next(e for e in self.dynamics._events if isinstance(e, MacrophageDestroysBacterium) and
                        e._macrophage_type == MACROPHAGE_ACTIVATED and
                        e._bacterium_type == BACTERIUM_EXTRACELLULAR_DORMANT)
        self.assertEqual(ma_kills_bed._reaction_parameter, params[TBDynamics.RP_MA_DESTROY_BACTERIA])
        self.assertEqual(ma_kills_bed._half_sat, params[TBDynamics.HALF_SAT_MA_DESTROY_BACTERIA])
        ...
        self.assertEqual(dc_infect._half_sat, params[TBDynamics.HALF_SAT_DCI_MATURATION])
        ...
        self.assertEqual(mac_act_tc._half_sat, params[TBDynamics.HALF_SAT_MR_ACTIVATION_BY_TCELL])
        ...
        self.assertEqual(mac_act_bac._half_sat, params[TBDynamics.HALF_SAT_MR_ACTIVATION_BY_BACTERIA])
        ...
        self.assertEqual(mi_burst._sigmoid, params[TBDynamics.SIGMOID_BIM_REPLICATION])
        self.assertEqual(mi_burst._capacity, params[TBDynamics.MACROPHAGE_CAPACITY])
        ...
        self.assertEqual(mi_death_tcell._half_sat, params[TBDynamics.HALF_SAT_TA_KILL_MI])
        ...
        self.assertEqual(mac_infect._half_sat, params[TBDynamics.HALF_SAT_MR_INFECTION])
        ...
        self.assertEqual(tc_act._half_sat, params[TBDynamics.HALF_SAT_TCELL_ACTIVATION])
